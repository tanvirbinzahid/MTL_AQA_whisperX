...
class MocoC3D(nn.Module):
    def __init__(self, feat_dim=8192, proj_hidden=2048, proj_out=128):
        super().__init__()
    ...
    @torch.no_grad()
    def _momentum_update_key_encoder(self, m: float):
...
class MoCoQueue:
    def __init__(self, dim=128, K=4096):
        self.dim = dim
        self.K = K
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        latest_path = os.path.join(args.output_dir, 'moco_c3d_latest.pth')
        save_checkpoint(latest_path, model, optimizer, epoch, args, best_loss, queue)

        if avg_loss < best_loss:
            best_loss = avg_loss
            best_path = os.path.join(args.output_dir, 'moco_c3d_best.pth')
            save_checkpoint(best_path, model, optimizer, epoch, args, best_loss, queue)

        if epoch % args.save_every == 0:
            ep_path = os.path.join(args.output_dir, f'moco_c3d_epoch_{epoch:03d}.pth')
            save_checkpoint(ep_path, model, optimizer, epoch, args, best_loss, queue)

    backbone_only = os.path.join(args.output_dir, 'moco_c3d_backbone_only.pth')
    torch.save(model.encoder_q.state_dict(), backbone_only)
